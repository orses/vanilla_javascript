(() => {
  function renderRatingStars() {
    const starsContainer = document.querySelectorAll('.rating__stars');

    starsContainer.forEach((cv, i) => {
      const starsIcon = [];
      const numStars = cv.dataset.stars;
      ...
      cv.innerHTML = `<div>${starsIcon.join``}</div><div>Rated ${numStars} ${
        numStars > 1 ? 'Stars' : 'Star'
      } in ${cv.dataset.topic} </div>`;
    });
  }
